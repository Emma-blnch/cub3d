...
// 	raws = count_lines(game->config.map);
// 	cols = ft_strlen(game->config.map[0]);
// 	for (int i = 1; i < raws; ++i)
// 	{
// 		if (ft_strlen(game->config.map[i]) > cols)
// 			cols = ft_strlen(game->config.map[i]);
// 	}
...
//  position en float(précis) dans map   *  tile_width en gros 
	...
	player_icon_size = game->win_width * 0.002;
	dy = -player_icon_size;
...
void	draw_ray(t_game *game, float ray_angle)
{
	...
	while (i < 1000 && !wall) // 1000 taille de rayon max choisie arbitrairement
	{
		...
		if (scaled_x >= 0 && scaled_x < game->win_width && 
			...
			put_pixel_to_img(&game->mlx, scaled_x, scaled_y, 0xFF0000);
		i++;
	}
}
